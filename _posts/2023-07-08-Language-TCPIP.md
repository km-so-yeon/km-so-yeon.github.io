...
TCP는 **가상회선 방식**을 사용하는 프로토콜입니다. 

데이터를 전송하기 전에 논리적 연결을 설정하고(가상회선), 각 패킷에 가상회선 식별번호를 포함합니다. 패킷은 전송된 순서대로 도착하고 모든 패킷이 전송되면 가상회선이 해제됩니다. 3-way handshaking과정을 통해 연결을 설정하고 4-way handshaking을 통해 해제합니다.
...
데이터를 **데이터그램 단위로 처리**하는 프로토콜입니다. 

TCP와는 다르게 비연결형 프로토콜로, 연결을 설정하거나 해제하는 과정이 존재하지 않습니다. 또한 해당 패킷의 순서를 표시하거나, 의도한 대로 도착했는지 여부를 확인하지 않고 패킷(데이터그램)을 대상 컴퓨터로 직접 보내는 간단한 방식을 사용합니다. 

#### 특징

- 헤더의 체크섬 필드를 통해 최소한의 오류만 검출하기 때문에 신뢰성이 낮습니다.
- 데이터를 전송하기 전에 논리적 연결이 설정되지 않으므로 TCP보다 일반적으로 속도가 빠릅니다. 
- 순차전송 보장 X, 흐름제어 X, 혼잡제어 X
- 커넥션을 끊은 상태로 다방향으로 연결을 합니다. 그래서 각각 패킷들이 독립적이고, 각각 독립적인 경로로 전송된다.
